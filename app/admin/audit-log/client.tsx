...
export function AdminAuditLogClient({ initialData }: { initialData: any }) {
  const [searchQuery, setSearchQuery] = useState("");
  const [actionFilter, setActionFilter] = useState("all");

  const filtered = initialData.entries.filter((entry: any) => {
    const matchesSearch =
      (entry.actor?.full_name || "").toLowerCase().includes(searchQuery.toLowerCase()) ||
      (entry.ticket?.ticket_number || "").toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="mt-1 flex flex-wrap items-center gap-2">
                    <Badge variant="secondary" className="text-[10px] font-mono">
                      {entry.ticket?.ticket_number || "System"}
                    </Badge>
      ...
      <div className="flex items-center justify-between text-xs text-muted-foreground">
        <span>
          Showing {filtered.length} of {initialData.total} entries
        </span>
