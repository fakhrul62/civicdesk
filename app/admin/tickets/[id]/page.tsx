...
export default function AdminTicketDetailPage({
  ...
  const allMessages = mockMessages.filter((m) => m.ticket_id === id);
  const [messages, setMessages] = useState(allMessages);

  const ticketAudit = mockAuditLog.filter((a) => a.ticket_id === id);

  const timeline = ticket?.ticket_number === "CVD-2026-0001"
    ? mockTimeline
    : [{ id: "gen-1", status: "submitted" as TicketStatus, description: "Complaint submitted", actor: ticket?.citizen_name || "", created_at: ticket?.created_at || "" }];
  ...
  const handleSend = () => {
    ...
    setMessages((prev) => [...prev, {
      id: `new-${Date.now()}`, ticket_id: id, sender_id: "u6", sender_name: "Shahida Rahman",
      sender_role: "admin", body: newMessage, is_internal: isInternal, created_at: new Date().toISOString(),
    }]);
    setNewMessage("");
  };
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Submitted by {ticket.citizen_name} · {ticket.category} · {ticket.department}
            </p>
          </div>
        </div>
      </div>
          ...
          <Tabs defaultValue="messages">
            ...
            <TabsContent value="messages" className="mt-4 space-y-4">
              <div className="space-y-3">
                {messages.map((msg) => (
                  <div key={msg.id} className={cn("flex gap-3", msg.sender_role === "citizen" && "flex-row-reverse")}>
                    <Avatar className="h-7 w-7 shrink-0">
                      <AvatarFallback className="text-[10px] bg-primary/10 text-primary">
                        {msg.sender_name.split(" ").map((n) => n[0]).join("")}
                      </AvatarFallback>
                    </Avatar>
                    <div className={cn("max-w-[80%] rounded-lg border p-3", msg.is_internal ? "border-amber-500/30 bg-amber-500/5" : msg.sender_role === "citizen" ? "bg-primary/5 border-primary/20" : "bg-card")}>
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-medium">{msg.sender_name}</span>
                        <Badge variant="secondary" className="text-[10px] h-4 px-1">{msg.sender_role}</Badge>
              ...
              <div className="rounded-lg border bg-card p-3">
                ...
                <div className="mt-2 flex items-center justify-between">
                  ...
                  <Button size="sm" onClick={handleSend} className="gap-1.5"><Send className="h-3.5 w-3.5" /> Send</Button>
                </div>
              </div>
            </TabsContent>
                  ...
                  {timeline.map((event, i) => (
                    <div key={event.id} className="flex gap-3">
                      ...
                      <div className="pb-6">
                        ...
                        <p className="mt-0.5 text-xs text-muted-foreground">{event.actor} · {formatDateTime(event.created_at)}</p>
                      </div>
                    </div>
                  ))}
                </CardContent>
              ...
              <Card className="border">
                <CardContent className="p-0">
                  <div className="divide-y">
                    {ticketAudit.length > 0 ? ticketAudit.map((entry) => (
                      <div key={entry.id} className="flex items-start gap-3 p-3">
                        <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-muted text-[10px] font-medium">{entry.actor_name.split(" ").map(n => n[0]).join("")}</div>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm"><span className="font-medium">{entry.actor_name}</span> <span className="text-muted-foreground">{entry.action}</span></p>
              ...
              <Card className="border">
                <CardContent className="p-6 text-center text-sm text-muted-foreground">
                  ...
                  <div className="mt-3 flex flex-wrap justify-center gap-2">
                    <Badge variant="secondary" className="gap-1"><Paperclip className="h-3 w-3" /> pothole_photo_1.jpg</Badge>
                    <Badge variant="secondary" className="gap-1"><Paperclip className="h-3 w-3" /> damage_report.pdf</Badge>
                  </div>
                </CardContent>
              </Card>
            </TabsContent>
        ...
        <div className="space-y-4">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <Select value={assignedAgent} onValueChange={setAssignedAgent}>
                  ...
                  <SelectContent>
                    {mockAgentStats.map((a) => (<SelectItem key={a.id} value={a.id}>{a.name}</SelectItem>))}
                  </SelectContent>
                </Select>
              </div>
          ...
          <Card className="border">
            ...
            <CardContent className="space-y-3 text-sm">
              <div className="flex items-start gap-2"><Tag className="mt-0.5 h-3.5 w-3.5 text-muted-foreground" /><div><p className="text-xs text-muted-foreground">Category</p><p className="font-medium">{ticket.category}</p></div></div>
              <div className="flex items-start gap-2"><MapPin className="mt-0.5 h-3.5 w-3.5 text-muted-foreground" /><div><p className="text-xs text-muted-foreground">Location</p><p className="font-medium">{ticket.location}</p></div></div>
              <div className="flex items-start gap-2"><User className="mt-0.5 h-3.5 w-3.5 text-muted-foreground" /><div><p className="text-xs text-muted-foreground">Citizen</p><p className="font-medium">{ticket.citizen_name}</p></div></div>
              <Separator />
